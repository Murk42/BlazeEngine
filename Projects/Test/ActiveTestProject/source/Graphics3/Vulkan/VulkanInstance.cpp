...
namespace Graphics3::Vulkan
{
    ...
    VulkanInstance::VulkanInstance()
        : instance(VK_NULL_HANDLE), initWindow(nullptr)
...
#endif
    {
        ...
        if (windowSupport)
        {
            initWindow = (SDL_Window*)CreateWindowHandle("", Vec2i(INT_MAX), Vec2i(640, 480), WindowHandleGraphicsaAPI::Vulkan);

            if (initWindow == nullptr)            
                Debug::Logger::LogFatal("Blaze Vulkan API", "SDL_CreateWindow failed: \"" + (String)SDL_GetError() + "\"\n");                            

            uint32_t windowExtensionCount = 0;
            SDL_Vulkan_GetInstanceExtensions(initWindow, &windowExtensionCount, nullptr);
            Array<const char*> windowExtensions { windowExtensionCount };
            SDL_Vulkan_GetInstanceExtensions(initWindow, &windowExtensionCount, windowExtensions.Ptr());
    ...
    SDL_Window* VulkanInstance::CreateWindow()
    {
        if (initWindow != nullptr)
        {
        ...
        return (SDL_Window*)CreateWindowHandle("", Vec2i(INT_MAX), Vec2i(640, 480), WindowHandleGraphicsaAPI::Vulkan);
    }

    void VulkanInstance::FreeWindow(SDL_Window* window)
    {        
