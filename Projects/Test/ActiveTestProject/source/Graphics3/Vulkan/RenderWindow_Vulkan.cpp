...
namespace Graphics3::Vulkan
{
		...
		: graphicsContext(renderWindow.GetGraphicsContext()), renderWindow(&renderWindow), swapchain(VK_NULL_HANDLE), size(), format(), retired(false)
	{
		...
		VkExtent2D extent = ChooseSwapchainExtent(swapChainSupport.capabilities, (SDL_Window*)renderWindow.GetWindow()->GetHandle());
	...
	RenderWindow_Vulkan::RenderWindow_Vulkan(GraphicsContext_Vulkan* graphicsContext, String title, Vec2i size)
		: graphicsContext(graphicsContext), window(nullptr), surface(NULL), framebufferIndex(0), newSwapchainRequired(false), engineWindow(nullptr)
	{
		if (!graphicsContext)
			return;

		graphicsContext->CreateWindowAndSurface(window, surface);

		engineWindow = new Window((WindowHandle)window);

		engineWindow->resizedEventDispatcher.AddHandler(*this);
		engineWindow->minimizedEventDispatcher.AddHandler(*this);
	...
	void RenderWindow_Vulkan::Clear()
	{
		if (!graphicsContext)
			return;		
	...
	RenderWindowFramebuffer_Vulkan& RenderWindow_Vulkan::AquireNextFramebuffer(Semaphore_Vulkan& signalSemaphore)
	{								
