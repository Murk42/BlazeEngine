...
namespace Graphics2
{
    ...
    Vulkan_Swapchain::Vulkan_Swapchain(Vulkan_RenderContext* renderContext)
        : renderContext(renderContext), swapChain(nullptr), renderPass(nullptr), width(0), height(0), imageCount(0), currentIndex(0)
    {
    }
    ...
    void Vulkan_Swapchain::Create(RenderWindow* renderWindow)
    {
        Vulkan_RenderWindow* vulkanRenderWindow = (Vulkan_RenderWindow*)renderWindow;

        SwapChainSupportDetails swapChainSupport = QuerySwapChainSupport(renderContext->physicalDevice, vulkanRenderWindow->surface);
        ...
        int width, height;
        SDL_Vulkan_GetDrawableSize(vulkanRenderWindow->window, &width, &height);
        ...
        createInfo.surface = vulkanRenderWindow->surface;
        ...
        framebuffers.ResizeWithFunction(imageCount,
            [&](Vulkan_Framebuffer* framebuffer, uint index) {
                std::construct_at(framebuffer, swapChainFramebuffers[index]);
            });
    ...
    void Vulkan_Swapchain::Destroy()
    {
        ...
        for (uint i = 0; i < imageCount; i++) {
            vkDestroyFence(renderContext->device, fences[i], nullptr);
        }
    ...
    Framebuffer& Vulkan_Swapchain::AdvanceFramebuffers(Semaphore* semaphore)
    {       
        currentIndex = (currentIndex + 1) % imageCount;
        
        vkWaitForFences(renderContext->device, 1, &fences[currentIndex], VK_TRUE, UINT64_MAX);
        vkResetFences(renderContext->device, 1, &fences[currentIndex]);

        uint32 imageIndex;
        VkResult result = vkAcquireNextImageKHR(renderContext->device, swapChain, UINT64_MAX, ((Vulkan_Semaphore*)semaphore)->semaphore, fences[currentIndex], &imageIndex);
        ...
        return (Framebuffer&)framebuffers[imageIndex];
    }
}
