...
namespace Blaze::UI
{
	...
	void InputManager::SubscribeToWindow(Window& window)
	{
		...
		UpdateScreenMousePos(window.GetMousePos(), window.GetSize());
	}
	...
	void InputManager::UpdateScreenMousePos(Vec2f windowPos, Vec2u windowSize)
	{
		windowPos.y = float(windowSize.y) - windowPos.y;
		...
		*/
		...
		for (;!it.IsNull(); --it)
		{
			...
			if (InputNode* inputNode = dynamic_cast<InputNode*>(it.Ptr()))
			{
				...
				if (newHit <= 0)
				{
					//Only set the hit status for the nodes that don't block hits.The hit status of the blocking node should be set AFTER reseting the hit status of the blocked nodes.
					inputNode->SetMouseHitStatus(newHit);
				}

				if (newHit != 0 && inputNode->GetMouseMotionReportPolicy() == InputNode::MouseMotionReportPolicy::ReportWhileMouseInside)
				{
		...
		if (blockingNode != nullptr)
			if (InputNode* inputNode = dynamic_cast<InputNode*>(blockingNode))
				inputNode->SetMouseHitStatus(blockingNodeHitStatus);
	...
	void InputManager::InputPostUpdateEvent(const Input::InputPostUpdateEvent& event)
	{
	}
	...
	void InputManager::MouseMotionEvent(const Input::MouseMotionEvent& event)
	{
		UpdateScreenMousePos(event.pos, event.window->GetSize());
	...
	void InputManager::MouseScrollEvent(const Input::MouseScrollEvent& event)
	{
	...
	void InputManager::MouseButtonDownEvent(const Input::MouseButtonDownEvent& event)
	{
	...
	void InputManager::MouseButtonUpEvent(const Input::MouseButtonUpEvent& event)
	{
	...
	void InputManager::KeyDownEvent(const Input::KeyDownEvent& event)
	{
		if (selectedNode == nullptr)
			return;
	...
	void InputManager::KeyUpEvent(const Input::KeyUpEvent& event)
	{
		if (selectedNode == nullptr)
			return;
	...
	void InputManager::TextInputEvent(const Input::TextInputEvent& event)
	{
		if (selectedNode == nullptr)
			return;
