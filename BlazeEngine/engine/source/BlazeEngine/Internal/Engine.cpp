...
{	
	...
	{
		std::lock_guard<std::mutex> lk(engine->ProcessInfo.threadIDsMutex);
		auto it = engine->ProcessInfo.threadIDMap.find(id);
		if (it == engine->ProcessInfo.threadIDMap.end())
			return (engine->ProcessInfo.threadIDMap[id] = engine->ProcessInfo.threadIDMap.size());
		return engine->ProcessInfo.threadIDMap[id];
	}
