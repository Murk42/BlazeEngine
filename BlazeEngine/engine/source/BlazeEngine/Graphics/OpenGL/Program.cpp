...
namespace Blaze
{
	namespace OpenGL
	{
		...
		inline Program::Program(Program&& p) noexcept
			: id(0)
		{
			id = p.id;
			p.id = 0;
		...
		inline bool Program::LinkProgram()
		{
			glLinkProgram(id);
		...
		inline String Program::GetLinkingLog()
		{
			int lenght = 0;
		...
		inline uint Program::GetUniformCount() const
		{
			int count;
		...
		inline int Program::GetUniformLocation(const StringView& name)
		{
			return glGetUniformLocation(id, name.Ptr());
		}

		inline void Program::GetUniformData(uint index, String& name, uint& uniformSize, UniformType& uniformType) const
		{
			...
			name = std::move(String(ptr, maxUniformNameLenght, false));
			uniformSize = size;
			uniformType = (UniformType)type;				
		}

		inline uint Program::GetUniformBlockCount() const
		{		
		...
		inline void Program::GetUniformBlockData(uint index, int& location, String& name, uint& uniformBlockSize, std::vector<int>& memberIndicies) const
		{
			char* ptr = new char[maxUniformBlockNameLenght];
			int lenght;
			glGetActiveUniformBlockName(id, index, maxUniformBlockNameLenght, &lenght, ptr);
			name = std::move(String(ptr, lenght, false));
		...
		inline void Program::GetUniformBlockMemberData(int location, String& name, uint& size, UniformType& type, uint& offset)
		{
			...
			name = std::move(String(ptr, v2.uniformBlockVariableNameLenght, false));
			size = v2.uniformBlockVariableSize;
			type = (UniformType)v2.uniformBlockVariableType;
			offset = v2.uniformBlockVariableOffset;
		}

		inline void Program::SetUniform(int location, const int& value)
		{
			glProgramUniform1i(id, location, value);
		}

		inline void Program::SetUniform(int location, const Vec4f& value)
		{
			glProgramUniform4f(id, location, value.x, value.y, value.z, value.w);
		}

		inline void Program::SetUniform(int location, const Mat4f& value)
		{
		...
		{
			glUseProgram(id);
			engine->Renderer.usedProgram = (Program*)this;
		}

		inline void Program::Unuse()
		{
			glUseProgram(0);
			engine->Renderer.usedProgram = nullptr;
		}
		inline Program* Program::GetUsed()
		{
			return engine->Renderer.usedProgram;
		}

				
		inline Program& Program::operator=(Program&& p) noexcept
		{
			id = p.id;
			p.id = 0;
