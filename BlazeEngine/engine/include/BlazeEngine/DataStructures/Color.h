...
namespace Blaze
{
	...
	struct BLAZE_API Colorb
	{
		...
		constexpr Colorb(const float& r, const float& g, const float& b, const float& a = 1.0f) : r(r * 255.0f), g(g * 255.0f), b(b * 255.0f), a(a * 255.0f) { }

		constexpr inline bool operator==(const Colorb& c) const { return r == c.r && g == c.g && b == c.b && a == c.a; }
		constexpr inline bool operator!=(const Colorb& c) const { return r != c.r || g != c.g || b != c.b || a != c.a; }
		constexpr inline void operator=(const Color& c) { r = c.r * 255; g = c.g * 255; b = c.b * 255; a = c.a * 255; }
