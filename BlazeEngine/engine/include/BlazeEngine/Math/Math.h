...
namespace Blaze
{
	namespace Math
	{
		...
		template<typename T>
		constexpr inline Matrix<T, 4, 4> TransformMatrix(const Vec3<T>& pos, const Vec3<T>& scale, const Quat<T>& rot)
		{
			return TranslationMatrix(pos) * RotationMatrix(rot) * ScalingMatrix(scale);
		}
		template<typename T>
		constexpr inline Matrix<T, 4, 4> ViewMatrix(const Vec3<T>& pos, const Vec3<T>& scale, const Quat<T>& rot)
		{
			return RotationMatrix(rot.Conjugated()) * ScalingMatrix(scale) * TranslationMatrix(-pos);			
		}
	};
}