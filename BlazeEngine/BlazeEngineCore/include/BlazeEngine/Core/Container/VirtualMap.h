...
namespace Blaze
{
	...
	struct BLAZE_API VirtualMapNodeBase
	{
		using KeyType = Key;
		using BaseType = Base;
	...
	*/
	...
	class BLAZE_API VirtualMapIterator
	{
	public:
		using NodeBaseType = NodeBase;
		using VirtualMapConstIterator = VirtualMapIterator<const NodeBase>;
		using KeyType = typename NodeBase::KeyType;
		using BaseType = typename NodeBase::BaseType;
		template<typename T>
		using MatchConst = Conditional<IsConstType<NodeBase>, const T, T>;

		VirtualMapIterator();
		VirtualMapIterator(const VirtualMapIterator& other);
		VirtualMapIterator(const VirtualMapConstIterator& other) requires (!IsConstType<VirtualMapIterator>);
		VirtualMapIterator(MatchConst<VirtualMapData<KeyType, BaseType>>& virtualMapData, typename VirtualMapData<KeyType, BaseType>::Bucket* bucket, NodeBase* nodeBase);

		bool IsNull() const;

		template<typename Derived>
		MatchConst<Derived>* GetValue() const requires IsDerivedFrom<Derived, BaseType> || SameAs<BaseType, void>;
		...
		bool operator==(const VirtualMapConstIterator& other) const requires (!IsConstType<VirtualMapIterator>);
		bool operator!=(const VirtualMapIterator& other) const;
		bool operator!=(const VirtualMapConstIterator& other) const requires (!IsConstType<VirtualMapIterator>);

		VirtualMapIterator& operator=(const VirtualMapIterator& other);
		VirtualMapIterator& operator=(const VirtualMapConstIterator& other) requires (!IsConstType<VirtualMapIterator>);
	...
	private:
		...
		typename VirtualMapData<KeyType, BaseType>::Bucket* bucket;
		NodeBase* nodeBase;
	};
	...
	*/
	...
	class BLAZE_API VirtualMap : private VirtualMapData<Key, Base>
	{
	public:
		using KeyType = Key;
		using BaseType = Base;
		...
		\returns
			...
			with the same key.
		*/
		template<typename Derived, bool Replace = false, typename _Key, typename ... Args> requires (IsDerivedFrom<Derived, Base> || SameAs<Base, void>) && IsConstructibleFrom<Key, _Key>&& IsConstructibleFrom<Derived, Args...>
	...
	private:
		using Bucket = typename VirtualMapData<Key, Base>::Bucket;
