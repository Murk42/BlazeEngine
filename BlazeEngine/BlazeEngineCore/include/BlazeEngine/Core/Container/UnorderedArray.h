...
namespace Blaze
{
	...
	concept IsConvertibleToUnorderedArrayIterator =
		SameAs<RemoveConst<typename T2::template UnorderedArrayType>, RemoveConst<typename T1::template UnorderedArrayType>> &&
		(!(IsConstType<typename T1::template UnorderedArrayType> && !IsConstType<typename T2::template UnorderedArrayType>));
	...
	*/
	...
	class BLAZE_API UnorderedArrayIterator
	{
	public:
		using UnorderedArrayType = UnorderedArray;
		using ValueType = Conditional<IsConstType<UnorderedArray>, const typename UnorderedArray::template ValueType, typename UnorderedArray::template ValueType>;
	...
	*/
	...
	public:
		...
		UnorderedArray(const ArrayView<RemoveConst<T>>& arr);
		UnorderedArray(const ArrayView<const RemoveConst<T>>& arr);
		...
		*/
		...
		operator ArrayView<RemoveConst<T>>();

		UnorderedArray& operator=(const ArrayView<RemoveConst<T>>& other);
		UnorderedArray& operator=(const ArrayView<const RemoveConst<T>>& other);
