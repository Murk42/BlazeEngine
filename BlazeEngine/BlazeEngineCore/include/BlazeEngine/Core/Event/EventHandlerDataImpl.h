...
namespace Blaze
{
	...
	template<typename T>
	inline constexpr EventHandlerData<T>& EventHandlerData<T>::Set(EventHandler<T>& handlerObject)
	{
	...
	template<typename T>
	inline constexpr EventHandlerData<T>& EventHandlerData<T>::Set(void(*function)(const T&))
	{
		if (function == nullptr)
		{
			BLAZE_LOG_ERROR("Trying to set a static function but its pointer is nullptr");
			return;
		}
	...
	template<typename T>
	inline constexpr EventHandlerData<T>& EventHandlerData<T>::Set(void(*function)(const T&, void*), void* userData)
	{
		if (function == nullptr)
		{
			BLAZE_LOG_ERROR("Trying to set a static function but its pointer is nullptr");
	...
	template<typename T>
	template<auto MemberFunctionPointer> requires SameAs<MemberFunctionPointerTypeArgumentTypes<decltype(MemberFunctionPointer)>, TypeGroup<const T&>>
	inline constexpr EventHandlerData<T>& EventHandlerData<T>::Set(MemberFunctionPointerTypeClassType<decltype(MemberFunctionPointer)>& handlerObject)
	{
	...
	inline constexpr EventHandlerData<T>& EventHandlerData<T>::Set(MemberFunctionPointer memberFunctionPointer, MemberFunctionPointerTypeClassType<MemberFunctionPointer>& handlerObject)
	{
		invokingFunction = MemberFunctionInvokingFunction<MemberFunctionPointer>;
		data = &handlerObject;
		return *this;
