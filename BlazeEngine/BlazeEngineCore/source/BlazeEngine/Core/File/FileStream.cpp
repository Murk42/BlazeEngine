...
namespace Blaze
{
	...
	void FileStreamBase::Close()
	{
		...
		if (result == 0)
			BLAZE_WINDOWS_ERROR("CloseHandle failed with error :\"{}\"", Windows::GetErrorString(GetLastError()));
...
#pragma error
	...
	void FileStreamBase::Flush()
	{
		...
		if (file == nullptr)
		{
			BLAZE_LOG_ERROR("file is nullptr");
			return;
		}
		...
		if (result == 0)
			BLAZE_WINDOWS_ERROR("FlushFileBuffers failed with error :\"{}\"", Windows::GetErrorString(GetLastError()));
#else
#pragma error
#endif
	}
	bool FileStreamBase::MovePosition(intMem offset)
	{
		...
		if (result == 0)
		{
			BLAZE_WINDOWS_ERROR("SetFilePointerEx failed with error :\"{}\"", Windows::GetErrorString(GetLastError()));
			return false;
		}
...
#pragma error
	...
	uintMem FileStreamBase::GetSize() const
	{
		...
		if (result == 0)
		{
			BLAZE_WINDOWS_ERROR("GetFileSizeEx failed with error :\"{}\"", Windows::GetErrorString(GetLastError()));
...
#pragma error
	...
	uintMem FileWriteStream::Write(const void* ptr, uintMem byteCount)
	{
		...
		if (result == 0)
		{
			BLAZE_WINDOWS_ERROR("WriteFile failed with error :\"{}\"", Windows::GetErrorString(GetLastError()));
			return 0;
		}
...
#error
	...
	uintMem FileReadStream::Read(void* ptr, uintMem byteCount)
	{
		...
		if (result == 0)
			BLAZE_WINDOWS_ERROR("ReadFile failed with error :\"{}\"", Windows::GetErrorString(GetLastError()));
