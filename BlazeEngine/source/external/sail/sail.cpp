...
namespace Blaze
{
	...
	static void SailLogger(SailLogLevel level, const char* file, int line, const char* format, va_list args)
	{
		...
		logFunc("sail", Format(u8"sail ({}, line {}) level {}: \"{}\"", Path(StringView(file, strlen(file))).FileName(), line, levelString, message));
	}
	...
	TimingResult InitializeSail()
	{
		...
		BLAZE_LOG_INFO("Successfully initialized sail " SAIL_VERSION_STRING ". Available image formats: {}", StringView(", ").Join(imageFormats));
