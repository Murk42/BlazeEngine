...
namespace Blaze
{
	...
	TimingResult InitializeSDL()
	{
		...
		if (SDL_InitSubSystem(SDL_INIT_VIDEO) == false)
			BLAZE_LOG_FATAL("Failed to initialize the SDL library. SDL returned error: \"" + GetSDLError() + "\"");

		if (SDL_InitSubSystem(SDL_INIT_EVENTS) == false)
			BLAZE_LOG_FATAL("Failed to initialize the SDL library. SDL returned error: \"" + GetSDLError() + "\"");

		int version = SDL_GetVersion();
		BLAZE_LOG_INFO("Successfully initialized SDL {}.{}.{}", SDL_VERSIONNUM_MAJOR(version), SDL_VERSIONNUM_MINOR(version), SDL_VERSIONNUM_MICRO(version));
