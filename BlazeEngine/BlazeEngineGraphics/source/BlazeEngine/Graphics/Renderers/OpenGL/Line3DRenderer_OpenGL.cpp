...
namespace Blaze::Graphics::OpenGL
{
	...
	{
		auto vert = OpenGL::VertexShader(ShaderSources::line3d_vert);
		auto frag = OpenGL::FragmentShader(ShaderSources::line3d_frag);
		program.LinkShaders({ &vert, &frag });

		instanceBuffer.Allocate(nullptr, sizeof(Line3DInstance) * bufferLineCapacity,
			OpenGL::GraphicsBufferMapAccessFlags::Write,
			OpenGL::GraphicsBufferMapType::PersistentUncoherent
		);		

		vertexArray.EnableVertexAttribute(0);
		vertexArray.SetFloatVertexAttributeFormat(0, OpenGL::FloatVertexAttributeType::Float, 4, offsetof(Line3DInstance, color));
		...
		vertexArray.SetFloatVertexAttributeFormat(1, OpenGL::FloatVertexAttributeType::Float, 3, offsetof(Line3DInstance, p1));
		...
		vertexArray.SetFloatVertexAttributeFormat(2, OpenGL::FloatVertexAttributeType::Float, 3, offsetof(Line3DInstance, p2));
		...
		vertexArray.SetFloatVertexAttributeFormat(3, OpenGL::FloatVertexAttributeType::Float, 1, offsetof(Line3DInstance, width));
	...
	{		
		...
		while (true)
		{
			if (fence.BlockClient(1.0) == Blaze::Graphics::OpenGL::FenceReturnState::TimeoutExpired)
			{
				Debug::Logger::LogWarning("Blaze Graphics API", "Fence timeout");
				return;
			}
			
			Line3DInstance* instanceBufferMap = (Line3DInstance*)instanceBuffer.MapBufferRange(0, sizeof(Line3DInstance) * bufferInstanceCapacity, Blaze::Graphics::OpenGL::GraphicsBufferMapOptions::ExplicitFlush);
