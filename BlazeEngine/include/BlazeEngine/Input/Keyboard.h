...
namespace Blaze::Keyboard
{
    ...
    /*
        ...
        able to register multiple key presses or relases between two Input::Update() calls, only if they 
        happened at least once. Also key modifiers can't be registered corectly because they might have been
        down at a different time to the key being tested. For full control over key presses and releases use 
        event handling to handle key states.

        \param key - the key whose state to get
        \return A KeyState structure with information about the state of the key between two Input::Update()
        calls
    */
    BLAZE_API KeyFrameState GetFrameKeyState(Keyboard::Key key);
    /*
        Returns the current key state. By using this function an application wont be able to register 
        multiple key presses or relases between two calls to this function. Also key modifiers can't be 
        registered corectly because they might have been released by the time the function is called again.        
        For full control over key presses and releases use event handling to handle key states.

        \param key - the key whose state to get
        \return Returns true if the key is down at the moment of calling this function, false otherwise
    */
    BLAZE_API bool GetCurrentKeyState(Keyboard::Key key);
}