...
namespace Graphics3::Vulkan
{
		...
		: graphicsContext(renderWindow.GetGraphicsContext()), renderWindow(&renderWindow), swapchain(VK_NULL_HANDLE), size(), format(), retired(false)
	{
		...
		VkExtent2D extent = ChooseSwapchainExtent(swapChainSupport.capabilities, (SDL_Window*)window->GetHandle());
	...
	void RenderWindow_Vulkan::PresentFramebuffer(Dynamic::RenderWindowFramebufferBase& framebuffer, Array<Dynamic::SemaphoreBase*> waitSemaphores)
	{
		Array<Semaphore_Vulkan*> _waitSemaphores{ [&](auto it, auto index) { *it = (Semaphore_Vulkan*)waitSemaphores[index]; }, waitSemaphores.Count()};
		PresentFramebuffer((RenderWindowFramebuffer_Vulkan&)framebuffer, _waitSemaphores);
	}
	...
	{
		auto it = graphicsContext->dataMap.Find("RenderWindowPresentQueue");
		VkQueue queue = *it.GetValue<VkQueue>();
		...
		{
			Array<VkSemaphore> _waitSemaphores{ [&](auto it, auto index) {
				*it = waitSemaphores[index]->GetHandle();
			}, waitSemaphores.Count() };
