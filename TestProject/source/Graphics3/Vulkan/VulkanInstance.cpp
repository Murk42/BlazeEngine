...
namespace Graphics3::Vulkan
{
...
#endif
    {
        ...
        if (windowSupport)
        {
            initWindow = WindowSDL("", Vec2i(INT_MAX), Vec2i(640, 480), WindowHandleGraphicsaAPI::Vulkan);
    ...
    WindowSDL VulkanInstance::CreateWindow(StringView title, Vec2i pos, Vec2i size)
    {
        if (initWindow.IsNullWindow())
