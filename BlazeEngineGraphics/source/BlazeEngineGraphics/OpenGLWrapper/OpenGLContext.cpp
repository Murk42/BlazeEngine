...
namespace Blaze::Graphics::OpenGLWrapper
{
	...
	static VertexArray* selectedVertexArray = nullptr;
	static GraphicsBuffer* selectedArrayBuffer = nullptr;
	static GraphicsBuffer* selectedUniformBuffer = nullptr;
	static ShaderProgram* selectedProgram = nullptr;
	static Renderbuffer* selectedRenderbuffer = nullptr;
	static Framebuffer* selectedFramebuffer = nullptr;
	static Framebuffer* selectedDrawFramebuffer = nullptr;
	static Framebuffer* selectedReadFramebuffer = nullptr;

	static Texture1D* selectedTexture1D = nullptr;
	static Texture2D* selectedTexture2D = nullptr;
	static Texture3D* selectedTexture3D = nullptr;
	static Texture2DArray* selectedTexture2DArray = nullptr;
	static TextureCubemap* selectedTextureCubemap = nullptr;
	static TextureBuffer* selectedTextureBuffer = nullptr;
	...
	Result BindUniformBuffer(const GraphicsBuffer& buffer, uint binding)
	{
	...
	Result BindUniformBufferRange(const GraphicsBuffer& buffer, uint binding, uint offset, uint size)
	{
	...
	Result SelectTexture(Texture1D* obj)
	{
	...
	Texture1D* GetSelectedTexture1D()
	{
		return selectedTexture1D;
	}

	Result SelectTexture(Texture2D* obj)
	{
	...
	Texture2D* GetSelectedTexture2D()
	{
		return selectedTexture2D;
	}

	Result SelectTexture(Texture2DArray* obj)
	{
	...
	Texture2DArray* GetSelectedTexture2DArray()
	{
		return selectedTexture2DArray;
	}

	Result SelectTexture(TextureCubemap* obj)
	{
	...
	TextureCubemap* GetSelectedTextureCubemap()
	{
		return selectedTextureCubemap;
	}

	Result SelectTexture(Texture3D* obj)
	{
	...
	Texture3D* GetSelectedTexture3D()
	{
		return selectedTexture3D;
	}

	Result SelectTexture(TextureBuffer* obj)
	{
	...
	Result SelectVertexBuffer(GraphicsBuffer* obj)
	{
	...
	GraphicsBuffer* GetSelectedVertexBuffer()
	{
		return selectedArrayBuffer;
	}

	Result SelectIndexBuffer(GraphicsBuffer* obj)
	{
		return selectedVertexArray->SetIndexBuffer(*obj);
	}
	GraphicsBuffer* GetSelectedIndexBuffer()
	{
		return selectedVertexArray->GetIndexBuffer();
	}

	Result SelectUniformBuffer(GraphicsBuffer* obj)
	{
	...
	GraphicsBuffer* GetSelectedUniformBuffer()
	{
		return selectedUniformBuffer;
	}

	Result SelectVertexArray(VertexArray* obj)
	{
	...
	VertexArray* GetSelectedVertexArray()
	{
		return selectedVertexArray;
	}

	Result SelectProgram(ShaderProgram* obj)
	{
	...
	ShaderProgram* GetSelectedProgram()
	{
		return selectedProgram;
	}

	Result SelectFramebuffer(Framebuffer* obj)
	{
	...
	Framebuffer* GetSelectedFramebuffer()
	{
		return selectedFramebuffer;;
	}

	Result SelectDrawFramebuffer(Framebuffer* obj)
	{
	...
	Framebuffer* GetDrawSelectedFramebuffer()
	{
		return selectedDrawFramebuffer;
	}

	Result SelectReadFramebuffer(Framebuffer* obj)
	{
	...
	Framebuffer* GetReadSelectedFramebuffer()
	{
		return selectedReadFramebuffer;
	}

	Result SelectRenderbuffer(Renderbuffer* obj)
	{
	...
	Renderbuffer* GetSelectedRenderbuffer()
	{
	...
	Result SetRenderArea(Vec2i pos, Vec2u size)
	{
		renderArea = { pos, (Vec2i)size };
		glViewport(pos.x, pos.y, size.x, size.y);
		CHECK_OPENGL_RESULT();

		CHECK_RESULT(renderAreaChangedDispatcher.Call({ pos, (Vec2i)size }));
	...
	Result RenderInstancedPrimitiveArray(PrimitiveType type, uint startIndex, uint vertexCount, uint instanceCount)
	{
		glDrawArraysInstanced(OpenGLRenderPrimitive(type), startIndex, vertexCount, instanceCount);		
		CHECK_OPENGL_RESULT();
		return Result();
	}
	...
	Result SelectImage(uint slot, const Texture2D& texture, uint level, ImageAccess access, ImageFormat format)
	{
		glBindImageTexture(slot, texture.GetHandle(), level, GL_FALSE, 0, OpenGLImageAccess(access), OpenGLImageFormat(format));
		CHECK_OPENGL_RESULT();
		return Result();
	}
	Result SelectImage(uint slot, const Texture1D& texture, uint level, ImageAccess access, ImageFormat format)
	{
		glBindImageTexture(slot, texture.GetHandle(), level, GL_FALSE, 0, OpenGLImageAccess(access), OpenGLImageFormat(format));
		CHECK_OPENGL_RESULT();
		return Result();
