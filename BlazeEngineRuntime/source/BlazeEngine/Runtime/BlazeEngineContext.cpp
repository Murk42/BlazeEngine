...
namespace Blaze
{
	...
	bool(*generalSDLEventWatchRelay)(void*, SDL_Event*) = nullptr;
	...
	BlazeEngineContext::BlazeEngineContext(BlazeEngineContextInitializationParameters parameters) :
		...
		, sentMovedWindowEvent(false), sentResizedWindowEvent(false)
#endif
	{
		ResetInputState();

		generalSDLEventWatchRelay = [](void* data, SDL_Event* event) -> bool {
			return ((BlazeEngineContext*)data)->SDLEventWatch(data, event);
			};
		SDL_AddEventWatch(generalSDLEventWatchRelay, this);
	}
	BlazeEngineContext::~BlazeEngineContext()
	{
		...
		SDL_RemoveEventWatch(generalSDLEventWatchRelay, this);
	}
	...
	void* BlazeEngineContext::GetSystemCursor(Input::CursorType type)
	{
	...
	bool BlazeEngineContext::ExecuteOnMainThread(void(*function)(void*), void* data, uint64 timeoutMilliseconds) const
	{
		if (separateClientThreadMode)
		{
			struct Parameters
	...
	Window* BlazeEngineContext::GetWindowFromWindowID(uint32 id)
	{
		std::lock_guard lg{ windowsMutex };
	...
	bool BlazeEngineContext::SDLEventWatch(void* _userData, void* _event)
	{
		SDL_Event* event = static_cast<SDL_Event*>(_event);
		BlazeEngineContext* context = static_cast<BlazeEngineContext*>(_userData);
		...
		case SDL_EVENT_WINDOW_MOVED: {
#ifdef BLAZE_PLATFORM_WINDOWS
			if (context->sentMovedWindowEvent && !context->separateClientThreadMode)
				break;
#endif
			...
			if (context->separateClientThreadMode)
				context->AddToClientEventStack(_event, &window->windowMovedEventDispatcher);
			else
				context->clientEventStack.Add("Blaze::WindowMovedEvent", [_event]() {
					auto event = _event;
					SDL_SyncWindow((SDL_Window*)event.window.GetHandle());
					event.pos = event.window.GetPos();
					event.window.windowMovedEventDispatcher.Call(event);
					});

#ifdef BLAZE_PLATFORM_WINDOWS
			context->sentMovedWindowEvent = true;
#endif
			break;
		}
		...
		case SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED: {
#ifdef BLAZE_PLATFORM_WINDOWS
			if (context->sentResizedWindowEvent && !context->separateClientThreadMode)
				break;
#endif
			...
			if (context->separateClientThreadMode)
				...
				context->clientEventStack.Add("Blaze::WindowResizedEvent", [_event]() {
					auto event = _event;
					SDL_SyncWindow((SDL_Window*)event.window.GetHandle());
					event.size = event.window.GetSize();
					event.window.windowResizedEventDispatcher.Call(event);
					});

#ifdef BLAZE_PLATFORM_WINDOWS
			context->sentResizedWindowEvent = true;
#endif
			break;
		}
