...
#include "BlazeEngine/Core/Threading/ThreadEventStack.h"
#include <bitset>
#include <mutex>
...
namespace Blaze
{
	...
	public:
		...
		void* GetSystemCursor(Input::CursorType type);
		...
		bool ExecuteOnMainThread(void(*function)(void*), void* data, uint64 timeoutMilliseconds) const;
	private:
		...
		bool sentResizedWindowEvent;
		bool sentMovedWindowEvent;
#endif
		...
		*/
		...
		Window* GetWindowFromWindowID(uint32 id);

		//Thread safe - can be called from any thread
		template<typename T>
		void AddToClientEventStack(const T& event, EventDispatcher<T>* dispatcher);

		static bool SDLEventWatch(void* data, void* event);
	};
