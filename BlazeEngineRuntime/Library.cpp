...
Result LibraryView::Set(std::string path)
{
	handle = GetModuleHandleA(path.c_str());

	if (handle == nullptr)
		return Result("Failed to set library (error code : " + std::to_string(GetLastError()) + ") with path: " + path + "\n");

	return Result();
}

void* LibraryView::GetFunction(std::string name, Result& result)
{
	void* ptr = GetProcAddress((HMODULE)handle, name.c_str());
