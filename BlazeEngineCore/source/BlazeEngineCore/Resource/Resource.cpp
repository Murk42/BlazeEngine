...
namespace Blaze
{
	...
	{
		if (referenceCount != 0)
			BLAZE_ENGINE_CORE_FATAL("Destroying a resource with a reference count that isn't 0");
	}
	...
	void ResourceBase::DecreaseReferenceCount()
	{
		if (referenceCount == 0)
		{
			BLAZE_ENGINE_CORE_FATAL("Decreasing a reference count of a resource that has its resource count 0");
			return;
		}
