...
namespace Blaze
{
	...
	template<typename T, AllocatorType Allocator>
	inline UnorderedArray<T, Allocator>::UnorderedArray(const ArrayView<RemoveConst<T>>& arr)
	{
		CopyUnsafe(arr.Ptr(), arr.Count());
	}
	template<typename T, AllocatorType Allocator>
	inline UnorderedArray<T, Allocator>::UnorderedArray(const ArrayView<const RemoveConst<T>>& arr)
	{
		CopyUnsafe(arr.Ptr(), arr.Count());
	}
	template<typename T, AllocatorType Allocator>
...
#ifdef BLAZE_CONTAINER_INVALIDATION_CHECK
		if (iteratorCount > 0)
			BLAZE_LOG_WARNING("Clearing an array while some iterators are referencing it");
#endif
...
#ifdef BLAZE_CONTAINER_INVALIDATION_CHECK
		if (iteratorCount > 0)
			BLAZE_LOG_WARNING("Changing an unordered array while some iterators are referencing it");
#endif
...
#ifdef BLAZE_CONTAINER_INVALIDATION_CHECK
		if (iteratorCount > 0)
			BLAZE_LOG_WARNING("Changing an array while some iterators are referencing it");
#endif
...
#ifdef BLAZE_INVALID_ITERATOR_CHECK
		if (count == 0)
			BLAZE_LOG_FATAL("UnorderedArray is empty");
#endif
	...
	template<typename T, AllocatorType Allocator>
	inline UnorderedArray<T, Allocator>::operator ArrayView<RemoveConst<T>>()
	{
		return UnorderedArrayView(ptr, count);
	}		
	template<typename T, AllocatorType Allocator>
	inline UnorderedArray<T, Allocator>& UnorderedArray<T, Allocator>::operator=(const ArrayView<RemoveConst<T>>& other)
	{
		std::destroy_n(ptr, count);
	...
	template<typename T, AllocatorType Allocator>
	inline UnorderedArray<T, Allocator>& UnorderedArray<T, Allocator>::operator=(const ArrayView<const RemoveConst<T>>& other)
	{
		std::destroy_n(ptr, count);
