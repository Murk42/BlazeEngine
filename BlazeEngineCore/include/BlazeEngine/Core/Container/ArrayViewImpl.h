...
namespace Blaze
{
	...
	template<typename T>
	inline constexpr ArrayView<T>::ArrayView(const StoredType* begin, const StoredType* end)
		: ptr(nullptr), count(0)
	{
	...
	template<typename T>
	inline constexpr auto ArrayView<T>::Ptr() const -> const StoredType*
	{
		return ptr;
	}
	template<typename T>
	inline constexpr auto ArrayView<T>::Begin() const -> const StoredType*
	{
		return ptr;
	}
	template<typename T>
	inline constexpr auto ArrayView<T>::End() const -> const StoredType*
	{
		return ptr + count;
	}
	template<typename T>
	...
	template<typename T>
	inline constexpr bool ArrayView<T>::operator==(const ArrayView<T>& other) const requires std::equality_comparable<T> || IsReferenceType<T>
	{
