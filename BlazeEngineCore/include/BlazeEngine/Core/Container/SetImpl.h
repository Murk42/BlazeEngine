...
namespace Blaze
{
		...
		for (Bucket* it = begin; it != end; ++it)
		{
			...
			while (node != nullptr)
			{
				...
				if (node->iteratorCount)
					BLAZE_LOG_WARNING("Clearing a set while a iterator is referencing it");
#endif
				Node* next = node->next;
				std::destroy_at(node);
				allocator.Free(node);
...
#ifdef BLAZE_INVALID_ITERATOR_CHECK
		if (iterator.IsNull())
			return BLAZE_LOG_ERROR("Trying to erase a set value with a null iterator");
#endif
	...
	template<SetValueType Value, HasherOf<Value> Hasher, AllocatorType Allocator>
	inline uint64 Set<Value, Hasher, Allocator>::Hash(const Value& value)
	{
	...
	inline Set<Value, Hasher, Allocator>::InsertResult Set<Value, Hasher, Allocator>::InsertWithHint(uintMem hash, Value&& value) requires IsConstructibleFrom<Value, Value&&>
	{
		...
		if (bucket == nullptr)
		{
			...
			BLAZE_LOG_FATAL("Trying to insert a set element but there are no buckets");
...
#ifdef BLAZE_CONTAINER_INVALIDATION_CHECK
		if (node->iteratorCount)
			BLAZE_LOG_WARNING("Erasing a set node while a iterator is referencing it");
#endif
