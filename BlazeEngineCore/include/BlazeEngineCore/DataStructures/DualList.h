...
namespace Blaze
{
	...
	concept IsConvertibleToDualListIterator =
		std::same_as<std::remove_const_t<typename T2::template DualListType>, std::remove_const_t<typename T1::template DualListType>> &&
		(!(std::is_const_v<typename T1::template DualListType> && !std::is_const_v<typename T2::template DualListType>));
	...
	/*
		...
		No other macros change the dual list iterator behaviour
	*/
	template<typename DualList>
	class BLAZE_CORE_API DualListIterator
	{
	public:
		using DualListType = DualList;
		using ValueType = std::conditional_t<std::is_const_v<DualList>, const typename DualList::template ValueType, typename DualList::template ValueType>;
	...
	private:
		using Node = typename DualList::template Node;
	...
	*/
	...
	class BLAZE_CORE_API DualList : private Allocator
	{
	public:
